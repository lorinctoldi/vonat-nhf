...
void Menetrend::addVonat(Vonat v)
{
  ...
  for (int i = 0; i < vonatok_szama - 1; ++i)
...
void Menetrend::changeVonat(Vonat v, int index)
{
...
Vonat &Menetrend::getVonat(int index) const
{
...
void Menetrend::removeVonat(int index)
{
  if (index >= vonatok_szama)
  {
  ...
  for (int i = index; i < vonatok_szama - 1; ++i)
  {
...
void Menetrend::createJegy(std::string indulo, std::string erkezo, int indulo_ora, int indulo_perc, double discountOrFee, const std::string &tipus)
{
  ...
  int leghamarabb_indulo_vonat = -1;
  int min_ido = -1;
  for (int i = 0; i < vonatok_szama; ++i)
  {
  ...
  int jegy_index = vonatok[leghamarabb_indulo_vonat].createJegy(indulo, erkezo, indulo_ora, indulo_perc, discountOrFee, tipus);
  ...
  for (int i = 0; i < vonatok_szama; ++i)
  {
    ...
    if ((indulo.empty() || (induloIndex != -1)) && (erkezo.empty() || (erkezoIndex != -1)))
    {
      ...
      else if ((induloIndex != -1))
      {
        ...
        for (int j = induloIndex; j < u.getAllomasokSzama(); ++j)
        {
